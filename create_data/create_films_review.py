...
print(f"Найдено {len(films_list)} фильмов для обработки.")

for i, film in enumerate(films_list):
    print(f"\n--- Обработка фильма {i + 1}/{len(films_list)}: {film.strip()} ---")
    try:
        resulting = process(film, prompt_template)
        logging.info(f'Итерация с фильмом {resulting[0]} удачно завершилась')
        print(f'Добавлен фильм {resulting[0]}')
    except Exception as e:
        print(f"Критическая ошибка при обработке фильма {film}: {e}")
